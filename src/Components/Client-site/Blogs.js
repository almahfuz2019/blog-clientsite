...
const Blogs = () => {
  ...
  return (
    <div className="mx-2">
      <h1 className="text-center text-xl sm:text-2xl md:text-3xl font-bold my-5 mx-5">
            ...
            <div className=" w-full bg-base-200 shadow" key={singleBlog._id}>
              ...
              <div className="px-2 pb-5">
                ...
                <Link
                  ...
                  className="card-title font-bold hover:text-primary cursor-pointer text-xl my-1 hover:underline hover:underline-offset-4"
                >
                  {singleBlog.title.slice(0, 25)}
                </Link>
                <p className="mb-2 text-xs">
                  By <span className="font-bold">{singleBlog.authorName}</span>,
                ...
                <Link
                  to={`/blog/${singleBlog._id}`}
                  className="flex items-end hover:font-bold text-primary mt-3 hover:ease-in duration-300"
                >
                  Continue Reading{" "}
                  <BiRightArrowAlt className="text-xl font-extrabold" />{" "}
                </Link>
              </div>
            </div>
          ))}
      ...
      blogs.length === blogsCount.count? (
        ...
        <div className="mx-auto text-center mt-5">
          <button
            className="btn btn-primary w-64 btn-sm sm:btn-md"
            onClick={loadMore}
          >
            load more
          </button>
