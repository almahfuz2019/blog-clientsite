...
formData
     ...
     const handleAddProduct = data => {
        console.log(...data,description,image,date);
        axios.post("http://localhost:5000/createblog",{...data,description,image,date})
        .then(response => {
...
<div className="container px-5 sm:py-24 mx-auto flex sm:flex-nowrap flex-wrap ">
    <div className="border-primary border-2 md:w-1/2 bg-white flex flex-col md:mx-auto w-full md:py-8 mt-8 md:mt-0  rounded-md p-5">
     ...
     title-font font-semibold">Add a Blog</h2>
...
<div className="relative mb-4">
        <label for="name" className="leading-7 text-sm text-gray-600">Author Name</label>
        <input  defaultValue={user?.displayName || ""} type="text" {...register("authorName")} className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
    </div>
<div className="relative mb-4">
        <label for="name" className="leading-7 text-sm text-gray-600">Author Email</label>
        <input  defaultValue={user?.email || ""} type="text" {...register("authorEmail")} className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
    </div>
    <div className="relative mb-4">
    ...
    <div className="relative mb-4">
        ...
        <ReactQuill className='h-32 mb-5' theme="snow"  onChange={setDescription} />
        ...
        className="select w-full  border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 rounded">
{
    ...
    <div className="relative mb-4">
        <label className="leading-7 text-sm text-gray-600">select blog image</label>
        <input type="file"  className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out" onChange={firstImageUpload} required />
