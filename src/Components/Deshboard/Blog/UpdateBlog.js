...
const UpdateBlog = () => {
  ...
  const onSubmit = (formData) => {
     ...
     axios.put(`http://localhost:5000/updateblog/${id}`, {...formData,image})
       .then(response =>
...
<div className="relative mb-4">
     ...
     <input defaultValue={blog?.title} type="text" {...register("title", {
     })} className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
     
 </div>
<div className="relative mb-4">
     <label for="name" className="leading-7 text-sm text-gray-600">Author Name</label>
     <input defaultValue={blog.authorName} type="text" {...register("authorName")} className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"/>
    
 </div>
 <div className="relative mb-4">
     <label className="leading-7 text-sm text-gray-600">Keywords</label>
     <input name='keywords' defaultValue={blog.keywords} type="text" {...register("keywords")} className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out" />
     
 </div>

 <div className="relative mb-4">
     <label className="leading-7 text-sm text-gray-600"> Description</label>
     <textarea {...register("description")} id="message" name="message" className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out" defaultValue={blog.description}></textarea>
    
 </div>
 <div className="relative mb-4 mt-5">
     <label className="leading-7 text-sm text-gray-600">Select a category</label>
     <select defaultValue={blog.category}
    {...register('category')}
